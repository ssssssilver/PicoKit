...
const guideByHref: Record<string, ToolGuideDetails> = {
  ...
  "/image-compressor": guide(
    [text("准备最多 50 张 JPG、PNG 或 WebP；源文件合计建议不超过 250 MB。", "Prepare up to 50 JPG, PNG, or WebP images with no more than 250 MB of source files in total."), text("先决定统一输出格式、最长边、质量，以及 JPG/WebP 是否需要目标 KB。", "Decide on one output format, longest edge, quality, and an optional target KB for JPG or WebP.")],
  ...
  "/pdf-tools": guide(
    [text("单个 PDF 最大 150 MB；加密或损坏文件可能无法读取。", "Each PDF can be up to 150 MB; encrypted or damaged files may not open."), text("图片转 PDF 支持 JPG 和 PNG。", "Images-to-PDF supports JPG and PNG.")],
    [step("选择 PDF", "Choose a PDF", "默认的“整理页面”模式会读取总页数并生成页面列表；原文件不会被修改。", "The default Organize pages mode reads the page count and builds a page list without changing the source."), step("整理页面", "Organize pages", "使用每页右侧按钮上移、下移、顺时针旋转或删除；没有缩略图的后续页面仍可正常整理。", "Use each page's controls to move it up or down, rotate clockwise, or remove it. Later pages without thumbnails can still be organized."), step("设置导出标记", "Configure export marks", "按需添加底部页码并设置起始值，或添加文字水印和不透明度。", "Optionally add bottom page numbers with a starting value, or add a text watermark and opacity."), step("导出并复核", "Export and verify", "导出整理后的 PDF，重新打开检查页数、顺序、方向、页码与水印。", "Export the organized PDF and reopen it to verify page count, order, orientation, numbering, and watermark."), step("使用其他模式", "Use another mode", "需要时切换到合并、按页码提取/删除/旋转、图片转 PDF 或 PDF 转图片。", "When needed, switch to merge, page-range extraction/deletion/rotation, images to PDF, or PDF to images.")],
    ...
    [text("页面整理至少要保留一页；删除按钮在只剩一页时会停用。", "An organized PDF must retain at least one page; deletion is disabled when only one remains."), text("加密 PDF 需要先在有权限的软件中解锁。", "Encrypted PDFs must be unlocked first in authorized software."), text("超大 PDF 处理慢或内存不足时拆成较小批次；缩略图数量受限不影响全部页面导出。", "Split very large PDFs into smaller batches if processing is slow or memory-limited; the thumbnail limit does not restrict full-page export.")],
    7,
  ),
