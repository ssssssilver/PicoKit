...
const guideByHref: Record<string, ToolGuideDetails> = {
  "/ai-text-detector": guide(
    ...
    [text("不足 300 个字符时请补充完整段落，短文本不适合评分。", "If the text is under 300 characters, add complete paragraphs; short text is unsuitable for scoring."), text("检测无法启动时请刷新页面，并确认网络正常、浏览器没有拦截所需文件。", "If the model cannot start, refresh and confirm that the browser can load WebGPU/WASM and model assets."), text("翻译、专业模板或非英文内容可能误判，结果不能作为作者身份鉴定。", "Translations, professional templates, and non-English text can be misclassified; the result is not proof of authorship.")],
    7,
  ),
  "/ai-image-detector": guide(
    ...
    [step("选择图片", "Choose an image", "选择本地图片后会立即显示预览并自动开始检测，无需再次点击开始。", "Choose a local image to show its preview and start detection automatically, with no extra start step."), step("运行来源证据检查", "Run the provenance evidence check", "等待文件来源、可见平台标记和像素检测三个通道分别完成；需要时会自动进行增强与最终复核。", "Wait for file provenance, visible platform marks, and pixel checks to finish independently. Enhanced and final checks run automatically when needed."), step("先看可验证证据", "Start with verifiable evidence", "优先阅读 C2PA、EXIF、XMP 与生成器字段；这些是文件内证据，但缺失不代表图片一定来自相机。", "Review C2PA, EXIF, XMP, and generator fields first. These are file-level evidence, but their absence does not prove camera origin."), step("再看视觉线索与统计估计", "Then review visual clues and statistical estimates", "分别查看可见平台标记与多级像素复核结果，留意模型冲突，不要把模型百分比当成来源事实。", "Review visible platform marks and the multi-stage pixel checks separately, note model disagreement, and do not treat a model percentage as provenance fact."), step("导出证据报告", "Export the evidence report", "下载 PDF 报告，保存检测器版本、通道可用性、文件证据、各级模型输出与限制。", "Download the PDF report to retain detector versions, channel availability, file evidence, per-stage model outputs, and limitations.")],
    ...
    [text("部分检测不可用时仍可查看图片来源信息，但不要只凭这一项下结论。", "If the pixel model is unavailable, provenance evidence still works, but do not treat one channel as a final verdict."), text("缺少元数据不能证明图片来自相机，可能只是被截图或重新导出。", "Missing metadata does not prove camera origin; the image may have been captured or re-exported."), text("插画、强压缩图片和新型生成器更容易落入不确定区间。", "Illustrations, heavily compressed images, and new generators are more likely to be uncertain.")],
    8,
  ),
  "/one-click-ai-cleaner": guide(
    [text("只处理你有权编辑的 JPEG、PNG 或 WebP，并保留仍可能需要的原文件。", "Only process JPEG, PNG, or WebP images you have the right to edit, and keep any source file you may still need."), text("工具会清理支持的可见角标和相关元数据，再重新生成图片；不会伪造相机 EXIF，也不能保证通过所有平台检测。", "The tool cleans supported visible marks and provenance fields, then applies a light image-delivery normalization. It does not fabricate camera EXIF or change the real creation history.")],
    [step("选择图片", "Choose an image", "加载本地图片并检查预览，确认需要处理的可见角标清晰存在。", "Load the image and confirm that the visible mark is clearly present in the preview."), step("检查处理范围", "Review the processing scope", "工具会尝试处理 Gemini、豆包、即梦可见角标、来源字段，并进行轻量重采样、传感器噪声与重新编码。", "The tool attempts to process visible Gemini, Doubao, and Jimeng marks and provenance fields, then applies light resampling, sensor grain, and re-encoding."), step("确认处理权利", "Confirm processing rights", "阅读处理边界并确认你拥有编辑图片的权利；清理不会改变真实创作来源。", "Read the processing boundary and confirm your right to edit the image. Cleanup does not change its real creation history."), step("运行一键清理", "Run one-click cleanup", "点击“一键清理 AI 痕迹”，工具会自动修复、复检并在仍有残留时继续补清，无需手动框选。", "Select Clean AI traces in one click. The tool repairs, verifies, and automatically retries any supported residual mark without manual selection."), step("下载或复检", "Download or recheck", "直接预览并下载最终结果；需要时把结果送回 AI 图片检测再次检查。", "Preview and download the final result directly, or send it back to AI image detection for another check.")],
    [text("结果卡明确显示可见角标是否处理、文件来源字段是否清理，以及复检后剩余的文件信号数。", "The result card clearly reports whether a visible mark was processed, whether provenance fields were cleaned, and how many file signals remain after verification."), text("下载结果可以正常打开，复杂纹理中的修复区域没有明显重复、模糊或色块。", "The downloaded result opens correctly and repaired areas on complex textures do not show obvious repetition, blur, or blocks.")],
    [text("没有识别到可见角标时，仍会清理相关来源信息并重新导出；复检发现残留角标时会自动再处理一次。", "If no visible mark is recognized, provenance fields are still cleaned and image delivery is normalized. When verification finds a supported residual mark, cleanup retries automatically."), text("本站会复检可见平台标记，但不同外部检测器可能得出不同结论。", "This site rechecks visible platform marks, but external detectors may reach different conclusions."), text("多次自动清理后仍未通过复检时不会生成下载，请换用更清晰、未经重压缩的原图。", "If repeated automatic cleanup still fails verification, no download is created. Try a clearer source image that has not been heavily recompressed.")],
    6,
  ),
  ...
  "/remove-background": guide(
    [text("一次最多添加 30 张 JPG、PNG 或 WebP，单张最大 15 MB；人像、商品、动物和其他常见主体都能处理。", "Add up to 30 JPG, PNG, or WebP images, each up to 15 MB. People, products, animals, and other subjects use the same on-device removal capability."), text("第一次使用需要联网准备一下，之后会直接使用浏览器缓存；图片会逐张处理，避免占用过多内存。", "The first run prepares about 4.6 MB and can use WebGPU or WASM CPU. The queue always processes one image at a time to limit memory use.")],
    [step("建立批量队列", "Build the batch queue", "一次选择或拖入多张图片，检查每张原图预览、尺寸和文件名。", "Choose or drop multiple images, then check each original preview, dimensions, and filename."), step("开始批量去背景", "Start batch removal", "工具按队列顺序逐张处理；需要时可在当前图片完成后停止。", "The tool processes images in queue order. You can stop after the current image when needed."), step("检查并修正逐项结果", "Review and refine each result", "每个队列项并排显示原图和透明结果；点击某张结果即可补回主体、擦除残留背景并柔化边缘。", "Each queue item shows the source and transparent result side by side. Select a result to restore subject areas, erase leftover background, and soften edges."), step("制作成品图", "Finish the image", "可保留透明背景，或换成纯色、渐变、自定义图片和模糊原图；还可调整画布比例、主体位置、大小与阴影。", "Keep transparency or use a solid color, gradient, custom image, or blurred original. You can also adjust the canvas, subject position, scale, and shadow."), step("批量应用设置", "Apply settings to the batch", "确认当前效果后，可仅应用到当前图片，或把相同成品设置应用到全部已完成图片。", "After reviewing the result, apply it to the current image or reuse the same finish settings across all completed images."), step("继续批量快速修图", "Continue to batch quick editing", "点击队列底部的继续按钮，将全部成功结果一次带入批量快速修图。", "Use the continue button below the queue to pass every successful result to batch quick editing at once."), step("下载结果", "Download results", "逐张下载 PNG、WebP 或 JPG，或把全部成功结果打包为 ZIP。", "Download PNG, WebP, or JPG files individually, or package all successful results as a ZIP.")],
    ...
    [text("浏览器加速不可用时会自动改用本机处理，速度可能慢一些，无需手动切换。", "When WebGPU is unavailable, the tool automatically uses this device's CPU. Processing may take longer, with no manual switching required."), text("处理功能准备失败时请检查网络后重试；所需文件会自动校验。", "If on-device setup fails, check the connection and retry. The downloaded file is SHA-256 verified."), text("细发丝、玻璃、烟雾、细辐条、低对比边缘和严重遮挡可能仍需保留或移除画笔修正。", "Fine hair, glass, smoke, fine spokes, low-contrast edges, and severe occlusion may still need Keep or Remove brush corrections.")],
    8,
  ),
  ...
  "/pdf-tools": guide(
    [text("单个 PDF 最大 150 MB，一次添加的文件合计最大 300 MB；加密或损坏的文件可能无法读取。", "Each PDF can be up to 150 MB and the workspace up to 300 MB; encrypted or damaged files may not open."), text("一次最多添加 20 个 PDF、合计 1000 页；图片转 PDF 支持 JPG 和 PNG。", "The workspace supports up to 20 PDFs and 1,000 pages; Images-to-PDF supports JPG and PNG.")],
    [step("添加来源 PDF", "Add source PDFs", "选择或拖入一个或多个 PDF；文件会逐个读取并按来源颜色标记。", "Choose or drop one or more PDFs. Files are read one at a time in the background and marked by source color."), step("选择并整理页面", "Select and organize pages", "单击选择一页，使用 Ctrl/Cmd 多选或 Shift 连续选择；拖动页面调整顺序，也可批量旋转或删除。", "Select one page, use Ctrl/Cmd for multiple pages or Shift for a range, then drag to reorder or batch-rotate and remove pages."), step("撤销或管理来源", "Undo or manage sources", "使用撤销、重做和恢复原始顺序；也可整体移动、隐藏或移除来源 PDF。", "Use undo, redo, and reset order, or move, hide, and remove an entire source PDF."), step("设置导出标记", "Configure export marks", "按需添加底部页码和文字水印，并查看来源数、保留页数与删除页数。", "Optionally add bottom page numbers and a text watermark, then review source, kept-page, and removed-page counts."), step("导出并复核", "Export and verify", "导出整理后的完整 PDF，或只提取选中的页面；导出时可以取消，完成后也会自动核对文件。", "Export the complete workspace or selected pages only. You can cancel the background export and reopen the result to verify it.")],
    ...
    [text("PDF 至少要保留一页，不能删除全部页面。", "The workspace must retain at least one page; removing every page is blocked."), text("加密 PDF 需要先在有权限的软件中解锁。", "Encrypted PDFs must be unlocked first in authorized software."), text("超大 PDF 处理慢或内存不足时拆成较小批次；缩略图只在可见时生成，不影响全部页面导出。", "Split very large PDFs into smaller batches if processing is slow or memory-limited. Thumbnails are generated only when visible and do not limit full export.")],
    7,
  ),
