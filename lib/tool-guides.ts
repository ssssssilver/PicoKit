...
const guideByHref: Record<string, ToolGuideDetails> = {
  ...
  "/one-click-ai-cleaner": guide(
    [text("只处理你有权编辑的 JPEG、PNG 或 WebP，并保留仍可能需要的原文件。", "Only process JPEG, PNG, or WebP images you have the right to edit, and keep any source file you may still need."), text("该工具清理支持的可见角标和文件来源字段，不移除 SynthID 等不可见水印，也不保证改变像素检测结果。", "This tool cleans supported visible marks and file provenance fields. It does not remove invisible watermarks such as SynthID or guarantee a different pixel-detector result.")],
    [step("从检测结果进入", "Continue from detection", "AI 图片检测判定为 AI 后点击“一键去 AI 标记”，当前图片会通过浏览器本地存储交接；也可以直接选择图片。", "After AI image detection returns an AI verdict, select Clean AI marks in one click. The image is handed off through browser-local storage, or you can choose one directly."), step("检查清理范围", "Review the cleanup scope", "确认工具将尝试处理 Gemini、豆包、即梦可见角标，以及 AI 元数据、C2PA 与 Made with AI 文件字段。", "Confirm that the tool will try to process visible Gemini, Doubao, and Jimeng marks plus AI metadata, C2PA, and Made with AI fields."), step("确认处理权利", "Confirm processing rights", "阅读处理边界并确认你拥有编辑图片的权利；清理标记不会改变真实创作来源。", "Read the processing boundary and confirm your right to edit the image. Cleaning marks does not change its real creation history."), step("运行一键清理", "Run one-click cleanup", "点击“一键清理 AI 标记”，等待可见角标修复、容器清理和输出复检依次完成。", "Select Clean AI marks in one click and wait for visible-mark repair, container cleanup, and output verification to finish."), step("比较、下载或复检", "Compare, download, or recheck", "放大比较原图与结果，下载清理图片；需要时把结果送回 AI 图片检测再次检查。", "Zoom in to compare the source and result, download the cleaned image, or send it back to AI image detection for another check.")],
    ...
    [text("没有识别到可见角标时仍会清理文件来源字段；如需处理位置变化或严重压缩的角标，请使用 AI 可见水印工具手动框选。", "If no visible mark is recognized, file provenance fields are still cleaned. Use manual selection in the Visible AI Watermark Tool for shifted or heavily compressed marks."), text("清理后仍被判为 AI 并不代表流程失败，像素模型可能依据图像内容而不是文件标记判断。", "A remaining AI verdict does not mean cleanup failed; pixel models may classify image content rather than file marks."), text("输出校验失败时不要使用结果，请保留原文件并刷新后重试。", "Do not use the result if output verification fails. Keep the source and retry after refreshing.")],
    6,
  ),
