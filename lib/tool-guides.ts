...
const guideByHref: Record<string, ToolGuideDetails> = {
  ...
  "/ai-image-detector": guide(
    ...
    [step("选择图片", "Choose an image", "上传区域只读取当前设备上的图片，并显示本地预览。", "Choose a local image; the page reads it on-device and shows a local preview."), step("运行来源证据检查", "Run the provenance evidence check", "等待文件来源、可见平台标记和像素检测三个通道分别完成；快速结果偏低或不确定时会自动进行增强复核。", "Wait for file provenance, visible platform marks, and pixel checks to finish independently. An enhanced check automatically reviews uncertain or low fast-check results."), step("先看可验证证据", "Start with verifiable evidence", "优先阅读 C2PA、EXIF、XMP 与生成器字段；这些是文件内证据，但缺失不代表图片一定来自相机。", "Review C2PA, EXIF, XMP, and generator fields first. These are file-level evidence, but their absence does not prove camera origin."), step("再看视觉线索与统计估计", "Then review visual clues and statistical estimates", "分别查看可见平台标记与两级像素结果，留意模型冲突，不要把模型百分比当成来源事实。", "Review visible platform marks and the two-stage pixel result separately, note model disagreement, and do not treat a model percentage as a provenance fact."), step("导出证据报告", "Export the evidence report", "导出 JSON 保存检测器版本、通道可用性、文件证据、各级模型输出与限制。", "Export JSON to retain detector versions, channel availability, file evidence, per-stage model outputs, and limitations.")],
  ...
  "/one-click-ai-cleaner": guide(
    ...
    [step("从检测结果进入", "Continue from detection", "AI 图片检测判定为 AI 后点击“一键去 AI 痕迹”，当前图片会通过浏览器本地存储交接；也可以直接选择图片。", "After AI image detection returns an AI verdict, select Clean AI traces in one click. The image is handed off through browser-local storage, or you can choose one directly."), step("检查处理范围", "Review the processing scope", "工具会尝试处理 Gemini、豆包、即梦可见角标、来源字段，并进行轻量重采样、传感器噪声与重新编码。", "The tool attempts to process visible Gemini, Doubao, and Jimeng marks and provenance fields, then applies light resampling, sensor grain, and re-encoding."), step("确认处理权利", "Confirm processing rights", "阅读处理边界并确认你拥有编辑图片的权利；清理不会改变真实创作来源。", "Read the processing boundary and confirm your right to edit the image. Cleanup does not change its real creation history."), step("运行一键清理", "Run one-click cleanup", "点击“一键清理 AI 痕迹”，等待可见角标修复、图像交付优化、容器清理和输出复检依次完成。", "Select Clean AI traces in one click and wait for visible-mark repair, image-delivery normalization, container cleanup, and output verification to finish."), step("比较、下载或复检", "Compare, download, or recheck", "放大比较原图与结果，下载清理图片；需要时把结果送回 AI 图片检测再次检查。", "Zoom in to compare the source and result, download the cleaned image, or send it back to AI image detection for another check.")],
    ...
    [text("没有识别到可见角标时仍会清理文件来源字段并优化图像交付；位置变化或严重压缩的角标可使用 AI 可见水印工具手动框选。", "If no visible mark is recognized, provenance fields are still cleaned and image delivery is normalized. Use manual selection in the Visible AI Watermark Tool for shifted or heavily compressed marks."), text("本站会复检可见平台标记，但不同外部检测器可能得出不同结论。", "This site rechecks visible platform marks, but external detectors may reach different conclusions."), text("输出校验失败时不要使用结果，请保留原文件并刷新后重试。", "Do not use the result if output verification fails. Keep the source and retry after refreshing.")],
    6,
  ),
