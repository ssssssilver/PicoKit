...
for (const value of [
  ...
  "Result",
  "Transparent result",
  // Preset labels are stored in a typed lookup before they are passed to
  // pick(), so keep them explicit in the generated locale catalog.
  "White studio",
  "Warm studio",
  "Cool studio",
  "Pastel glow",
  ...
  "Green nature",
  "City dusk",
  // Retain tested legacy labels even when the corresponding control is
...
function applyOverrides(output, language) {
  Object.assign(
    ...
    imageWorkspaceOverrides[language] ?? {},
    refinementZoomOverrides[language] ?? {},
    backgroundFinishOverrides[language] ?? {},
...
const legacyRegressionOverrides = {
  ja: { "Batch image optimizer": "画像一括最適化", "AI image check": "AI画像チェック", "Text workbench": "テキストワークベンチ", "Private PDF Organizer": "プライベート PDF 整理ツール" },
  ko: { "Batch image optimizer": "일괄 이미지 최적화", "AI image check": "AI 이미지 검사", "Text workbench": "텍스트 작업대", "Private PDF Organizer": "비공개 PDF 정리 도구", "Organize, Merge, and Convert PDFs": "PDF 정리, 병합 및 변환" },
  es: { "Batch image optimizer": "Optimizador de imágenes", "AI image check": "Comprobar imagen IA", "Text workbench": "Banco de trabajo de texto", "Private PDF Organizer": "Organizador privado de PDF" },
  pt: { "Batch image optimizer": "Otimizador de imagens", "AI image check": "Verificar imagem com IA", "Text workbench": "Bancada de texto", "Private PDF Organizer": "Organizador PDF privado" },
  id: { "Batch image optimizer": "Pengoptimal gambar", "AI image check": "Periksa gambar AI", "Text workbench": "Meja kerja teks", "Private PDF Organizer": "Pengelola PDF Privat" },
  de: { "Batch image optimizer": "Stapel-Bildoptimierer", "AI image check": "KI-Bildprüfung", "Text workbench": "Text-Workbench", "Private PDF Organizer": "Privater PDF-Organizer" },
  pl: { "Batch image optimizer": "Optymalizacja obrazów", "AI image check": "Sprawdź obraz AI", "Text workbench": "Warsztat tekstowy", "Private PDF Organizer": "Prywatny organizer PDF" },
  ru: { "Batch image optimizer": "Пакетная оптимизация", "AI image check": "Проверка AI-фото", "Text workbench": "Текстовый рабочий стол", "Private PDF Organizer": "Локальный органайзер PDF" },
  fr: { "Batch image optimizer": "Optimiseur d’images", "AI image check": "Vérifier une image IA", "Text workbench": "Établi textuel", "Private PDF Organizer": "Organisateur privé de PDF" },
  ar: { "Batch image optimizer": "محسّن الصور الدفعي", "AI image check": "فحص صورة بالذكاء الاصطناعي", "Text workbench": "مساحة أدوات النص", "Private PDF Organizer": "منظم PDF خاص" },
  tr: { "Batch image optimizer": "Toplu görsel iyileştirme", "AI image check": "AI görsel kontrolü", "Text workbench": "Metin çalışma tezgahı", "Private PDF Organizer": "Özel PDF Düzenleyici" },
}

const imageWorkspaceOverrides = {
  ja: {
    "{count} images in current tool": "現在のツール：{count}枚",
    "Choose tools for this batch": "このバッチで使うツールを選択",
    "Current batch · Output and download": "現在のバッチ · 出力とダウンロード",
    "Current batch · Quick editing": "現在のバッチ · クイック編集",
    "Cut out, refine, replace": "切り抜き・境界調整・背景変更",
    "Every tool is optional. Return to a panel and its queue remains available on this page.": "どのツールも任意です。パネルに戻ると、そのキューをこのページで引き続き利用できます。",
    "Every tool panel is optional, with no required processing order": "すべてのツールパネルは任意で、決まった処理順はありません",
    "Loading tools…": "ツールを読み込み中…",
    "One image batch, one workspace": "1つの画像バッチ、1つのワークスペース",
    "One image batch. One browser workspace.": "画像は1バッチ。ブラウザ内のワークスペースは1つ。",
    "Output & download": "出力とダウンロード",
    "Queues stay available": "キューを保持",
    "Quick edit": "クイック編集",
    "Remove & finish": "背景削除と仕上げ",
    "Send current batch to output ({count})": "現在のバッチを出力ツールへ送る（{count}枚）",
    "Send current results to quick editing ({count})": "現在の結果をクイック編集へ送る（{count}枚）",
    "Up to 30 images, with heavy work processed one item at a time": "最大30枚。負荷の高い処理は1枚ずつ実行",
    "Visited": "使用済み",
  },
  ko: {
    "{count} images in current tool": "현재 도구: {count}장",
    "Choose tools for this batch": "이 배치에 사용할 도구 선택",
    "Current batch · Output and download": "현재 배치 · 출력 및 다운로드",
    "Current batch · Quick editing": "현재 배치 · 빠른 편집",
    "Cut out, refine, replace": "배경 제거, 가장자리 보정, 배경 교체",
    "Every tool is optional. Return to a panel and its queue remains available on this page.": "모든 도구는 선택 사항입니다. 패널로 돌아오면 해당 대기열을 이 페이지에서 계속 사용할 수 있습니다.",
    "Every tool panel is optional, with no required processing order": "모든 도구 패널은 선택 사항이며 정해진 처리 순서가 없습니다",
    "Loading tools…": "도구 불러오는 중…",
    "One image batch, one workspace": "하나의 이미지 배치, 하나의 작업 공간",
    "One image batch. One browser workspace.": "이미지는 한 번에. 브라우저 작업 공간은 하나로.",
    "Output & download": "출력 및 다운로드",
    "Queues stay available": "대기열 유지",
    "Quick edit": "빠른 편집",
    "Remove & finish": "배경 제거 및 마무리",
    "Send current batch to output ({count})": "현재 배치를 출력 도구로 보내기 ({count}장)",
    "Send current results to quick editing ({count})": "현재 결과를 빠른 편집으로 보내기 ({count}장)",
    "Up to 30 images, with heavy work processed one item at a time": "최대 30장, 무거운 작업은 한 장씩 처리",
    "Visited": "사용함",
  },
  es: {
    "{count} images in current tool": "{count} imágenes en la herramienta actual",
    "Choose tools for this batch": "Elige las herramientas para este lote",
    "Current batch · Output and download": "Lote actual · Salida y descarga",
    "Current batch · Quick editing": "Lote actual · Edición rápida",
    "Cut out, refine, replace": "Quitar fondo, retocar bordes y sustituirlo",
    "Every tool is optional. Return to a panel and its queue remains available on this page.": "Todas las herramientas son opcionales. Al volver a un panel, su cola seguirá disponible en esta página.",
    "Every tool panel is optional, with no required processing order": "Todos los paneles son opcionales y no hay un orden obligatorio",
    "Loading tools…": "Cargando herramientas…",
    "One image batch, one workspace": "Un lote de imágenes, un espacio de trabajo",
    "One image batch. One browser workspace.": "Un lote de imágenes. Un solo espacio de trabajo en el navegador.",
    "Output & download": "Salida y descarga",
    "Queues stay available": "Las colas se conservan",
    "Quick edit": "Edición rápida",
    "Remove & finish": "Quitar fondo y finalizar",
    "Send current batch to output ({count})": "Enviar el lote actual a Salida ({count})",
    "Send current results to quick editing ({count})": "Enviar los resultados a Edición rápida ({count})",
    "Up to 30 images, with heavy work processed one item at a time": "Hasta 30 imágenes; las tareas pesadas se procesan de una en una",
    "Visited": "Usado",
  },
  pt: {
    "{count} images in current tool": "{count} imagens na ferramenta atual",
    "Choose tools for this batch": "Escolha as ferramentas para este lote",
    "Current batch · Output and download": "Lote atual · Saída e download",
    "Current batch · Quick editing": "Lote atual · Edição rápida",
    "Cut out, refine, replace": "Remover fundo, refinar bordas e substituir",
    "Every tool is optional. Return to a panel and its queue remains available on this page.": "Todas as ferramentas são opcionais. Ao voltar a um painel, a fila continua disponível nesta página.",
    "Every tool panel is optional, with no required processing order": "Todos os painéis são opcionais e não há ordem obrigatória",
    "Loading tools…": "Carregando ferramentas…",
    "One image batch, one workspace": "Um lote de imagens, um espaço de trabalho",
    "One image batch. One browser workspace.": "Um lote de imagens. Um único espaço de trabalho no navegador.",
    "Output & download": "Saída e download",
    "Queues stay available": "As filas são mantidas",
    "Quick edit": "Edição rápida",
    "Remove & finish": "Remover fundo e finalizar",
    "Send current batch to output ({count})": "Enviar o lote atual para Saída ({count})",
    "Send current results to quick editing ({count})": "Enviar os resultados para Edição rápida ({count})",
    "Up to 30 images, with heavy work processed one item at a time": "Até 30 imagens; tarefas pesadas são processadas uma por vez",
    "Visited": "Usado",
  },
  id: {
    "{count} images in current tool": "{count} gambar di alat saat ini",
    "Choose tools for this batch": "Pilih alat untuk kumpulan ini",
    "Current batch · Output and download": "Kumpulan saat ini · Output dan unduh",
    "Current batch · Quick editing": "Kumpulan saat ini · Edit cepat",
    "Cut out, refine, replace": "Hapus latar, rapikan tepi, ganti latar",
    "Every tool is optional. Return to a panel and its queue remains available on this page.": "Semua alat bersifat opsional. Saat kembali ke panel, antreannya tetap tersedia di halaman ini.",
    "Every tool panel is optional, with no required processing order": "Semua panel alat opsional tanpa urutan pemrosesan wajib",
    "Loading tools…": "Memuat alat…",
    "One image batch, one workspace": "Satu kumpulan gambar, satu ruang kerja",
    "One image batch. One browser workspace.": "Satu kumpulan gambar. Satu ruang kerja di browser.",
    "Output & download": "Output dan unduh",
    "Queues stay available": "Antrean tetap tersimpan",
    "Quick edit": "Edit cepat",
    "Remove & finish": "Hapus latar dan selesaikan",
    "Send current batch to output ({count})": "Kirim kumpulan saat ini ke Output ({count})",
    "Send current results to quick editing ({count})": "Kirim hasil saat ini ke Edit cepat ({count})",
    "Up to 30 images, with heavy work processed one item at a time": "Hingga 30 gambar; proses berat dijalankan satu per satu",
    "Visited": "Sudah digunakan",
  },
  de: {
    "{count} images in current tool": "{count} Bilder im aktuellen Werkzeug",
    "Choose tools for this batch": "Werkzeuge für diesen Stapel wählen",
    "Current batch · Output and download": "Aktueller Stapel · Ausgabe und Download",
    "Current batch · Quick editing": "Aktueller Stapel · Schnellbearbeitung",
    "Cut out, refine, replace": "Freistellen, Kanten verfeinern, Hintergrund ersetzen",
    "Every tool is optional. Return to a panel and its queue remains available on this page.": "Alle Werkzeuge sind optional. Beim Zurückkehren bleibt die Warteschlange des Bereichs auf dieser Seite erhalten.",
    "Every tool panel is optional, with no required processing order": "Alle Werkzeugbereiche sind optional; es gibt keine feste Reihenfolge",
    "Loading tools…": "Werkzeuge werden geladen…",
    "One image batch, one workspace": "Ein Bildstapel, ein Arbeitsbereich",
    "One image batch. One browser workspace.": "Ein Bildstapel. Ein Arbeitsbereich im Browser.",
    "Output & download": "Ausgabe und Download",
    "Queues stay available": "Warteschlangen bleiben erhalten",
    "Quick edit": "Schnellbearbeitung",
    "Remove & finish": "Freistellen und fertigstellen",
    "Send current batch to output ({count})": "Aktuellen Stapel an Ausgabe senden ({count})",
    "Send current results to quick editing ({count})": "Aktuelle Ergebnisse an Schnellbearbeitung senden ({count})",
    "Up to 30 images, with heavy work processed one item at a time": "Bis zu 30 Bilder; rechenintensive Aufgaben laufen einzeln",
    "Visited": "Verwendet",
  },
  pl: {
    "{count} images in current tool": "{count} obrazów w bieżącym narzędziu",
    "Choose tools for this batch": "Wybierz narzędzia dla tej partii",
    "Current batch · Output and download": "Bieżąca partia · Eksport i pobieranie",
    "Current batch · Quick editing": "Bieżąca partia · Szybka edycja",
    "Cut out, refine, replace": "Usuń tło, popraw krawędzie i zastąp tło",
    "Every tool is optional. Return to a panel and its queue remains available on this page.": "Każde narzędzie jest opcjonalne. Po powrocie do panelu jego kolejka nadal będzie dostępna na tej stronie.",
    "Every tool panel is optional, with no required processing order": "Każdy panel jest opcjonalny i nie ma wymaganej kolejności",
    "Loading tools…": "Ładowanie narzędzi…",
    "One image batch, one workspace": "Jedna partia obrazów, jeden obszar roboczy",
    "One image batch. One browser workspace.": "Jedna partia obrazów. Jeden obszar roboczy w przeglądarce.",
    "Output & download": "Eksport i pobieranie",
    "Queues stay available": "Kolejki są zachowywane",
    "Quick edit": "Szybka edycja",
    "Remove & finish": "Usuń tło i wykończ",
    "Send current batch to output ({count})": "Wyślij bieżącą partię do eksportu ({count})",
    "Send current results to quick editing ({count})": "Wyślij wyniki do szybkiej edycji ({count})",
    "Up to 30 images, with heavy work processed one item at a time": "Do 30 obrazów; cięższe zadania są wykonywane pojedynczo",
    "Visited": "Użyto",
  },
  ru: {
    "{count} images in current tool": "{count} изображений в текущем инструменте",
    "Choose tools for this batch": "Выберите инструменты для этой партии",
    "Current batch · Output and download": "Текущая партия · Экспорт и скачивание",
    "Current batch · Quick editing": "Текущая партия · Быстрое редактирование",
    "Cut out, refine, replace": "Удаление фона, коррекция краёв и замена фона",
    "Every tool is optional. Return to a panel and its queue remains available on this page.": "Все инструменты необязательны. При возврате к панели её очередь остаётся доступной на этой странице.",
    "Every tool panel is optional, with no required processing order": "Все панели необязательны, фиксированного порядка обработки нет",
    "Loading tools…": "Загрузка инструментов…",
    "One image batch, one workspace": "Одна партия изображений, одно рабочее пространство",
    "One image batch. One browser workspace.": "Одна партия изображений. Одно рабочее пространство в браузере.",
    "Output & download": "Экспорт и скачивание",
    "Queues stay available": "Очереди сохраняются",
    "Quick edit": "Быстрое редактирование",
    "Remove & finish": "Удалить фон и оформить",
    "Send current batch to output ({count})": "Передать текущую партию на экспорт ({count})",
    "Send current results to quick editing ({count})": "Передать результаты в быстрое редактирование ({count})",
    "Up to 30 images, with heavy work processed one item at a time": "До 30 изображений; тяжёлые задачи выполняются по одному",
    "Visited": "Использовано",
  },
  fr: {
    "{count} images in current tool": "{count} images dans l’outil actuel",
    "Choose tools for this batch": "Choisir les outils pour ce lot",
    "Current batch · Output and download": "Lot actuel · Export et téléchargement",
    "Current batch · Quick editing": "Lot actuel · Retouche rapide",
    "Cut out, refine, replace": "Détourer, affiner les bords et remplacer le fond",
    "Every tool is optional. Return to a panel and its queue remains available on this page.": "Chaque outil est facultatif. En revenant à un panneau, sa file reste disponible sur cette page.",
    "Every tool panel is optional, with no required processing order": "Tous les panneaux sont facultatifs, sans ordre imposé",
    "Loading tools…": "Chargement des outils…",
    "One image batch, one workspace": "Un lot d’images, un espace de travail",
    "One image batch. One browser workspace.": "Un lot d’images. Un seul espace de travail dans le navigateur.",
    "Output & download": "Export et téléchargement",
    "Queues stay available": "Les files sont conservées",
    "Quick edit": "Retouche rapide",
    "Remove & finish": "Détourer et finaliser",
    "Send current batch to output ({count})": "Envoyer le lot actuel vers l’export ({count})",
    "Send current results to quick editing ({count})": "Envoyer les résultats vers la retouche rapide ({count})",
    "Up to 30 images, with heavy work processed one item at a time": "Jusqu’à 30 images ; les tâches lourdes sont traitées une par une",
    "Visited": "Utilisé",
  },
  ar: {
    "{count} images in current tool": "{count} صورة في الأداة الحالية",
    "Choose tools for this batch": "اختر الأدوات لهذه الدفعة",
    "Current batch · Output and download": "الدفعة الحالية · التصدير والتنزيل",
    "Current batch · Quick editing": "الدفعة الحالية · تحرير سريع",
    "Cut out, refine, replace": "إزالة الخلفية وتحسين الحواف واستبدال الخلفية",
    "Every tool is optional. Return to a panel and its queue remains available on this page.": "كل أداة اختيارية. عند العودة إلى لوحة، تظل قائمة الانتظار الخاصة بها متاحة في هذه الصفحة.",
    "Every tool panel is optional, with no required processing order": "كل لوحات الأدوات اختيارية ولا يوجد ترتيب معالجة إلزامي",
    "Loading tools…": "جارٍ تحميل الأدوات…",
    "One image batch, one workspace": "دفعة صور واحدة، ومساحة عمل واحدة",
    "One image batch. One browser workspace.": "دفعة صور واحدة. مساحة عمل واحدة في المتصفح.",
    "Output & download": "التصدير والتنزيل",
    "Queues stay available": "تبقى قوائم الانتظار محفوظة",
    "Quick edit": "تحرير سريع",
    "Remove & finish": "إزالة الخلفية وإنهاء الصورة",
    "Send current batch to output ({count})": "إرسال الدفعة الحالية إلى التصدير ({count})",
    "Send current results to quick editing ({count})": "إرسال النتائج الحالية إلى التحرير السريع ({count})",
    "Up to 30 images, with heavy work processed one item at a time": "حتى 30 صورة؛ تُعالج المهام الثقيلة صورة تلو الأخرى",
    "Visited": "مُستخدم",
  },
  tr: {
    "{count} images in current tool": "Geçerli araçta {count} görsel",
    "Choose tools for this batch": "Bu toplu işlem için araçları seçin",
    "Current batch · Output and download": "Geçerli toplu işlem · Çıktı ve indirme",
    "Current batch · Quick editing": "Geçerli toplu işlem · Hızlı düzenleme",
    "Cut out, refine, replace": "Arka planı kaldır, kenarları düzelt ve değiştir",
    "Every tool is optional. Return to a panel and its queue remains available on this page.": "Tüm araçlar isteğe bağlıdır. Bir panele döndüğünüzde kuyruğu bu sayfada kullanılabilir kalır.",
    "Every tool panel is optional, with no required processing order": "Tüm araç panelleri isteğe bağlıdır; zorunlu bir işlem sırası yoktur",
    "Loading tools…": "Araçlar yükleniyor…",
    "One image batch, one workspace": "Tek görsel grubu, tek çalışma alanı",
    "One image batch. One browser workspace.": "Tek görsel grubu. Tarayıcıda tek çalışma alanı.",
    "Output & download": "Çıktı ve indirme",
    "Queues stay available": "Kuyruklar korunur",
    "Quick edit": "Hızlı düzenleme",
    "Remove & finish": "Arka planı kaldır ve tamamla",
    "Send current batch to output ({count})": "Geçerli toplu işlemi çıktıya gönder ({count})",
    "Send current results to quick editing ({count})": "Geçerli sonuçları hızlı düzenlemeye gönder ({count})",
    "Up to 30 images, with heavy work processed one item at a time": "En fazla 30 görsel; yoğun işler tek tek işlenir",
    "Visited": "Kullanıldı",
  },
}
...
const aiCleanerOverrides = {
  ja: {
    ...
    "One-click AI Trace Cleaner": "ワンクリックAI痕跡クリーナー",
    "Clean AI traces in one click": "AI痕跡をワンクリックで整理",
    "Image delivery normalization": "画像配信用の最適化",
    "Visible-mark verification": "可視マークの再検証",
    "Continue from detection": "検出結果から続ける",
  },
  ko: {
    "One-click AI Mark Cleaner": "원클릭 AI 표시 정리",
    "Clean AI marks in one click": "AI 표시를 한 번에 정리",
    "One-click AI Trace Cleaner": "원클릭 AI 흔적 정리",
    "Clean AI traces in one click": "AI 흔적을 한 번에 정리",
    "Image delivery normalization": "이미지 전송 최적화",
    "Visible-mark verification": "표시 재검사",
  ...
  es: {
    ...
    "One-click AI Trace Cleaner": "Limpiador de rastros de IA en un clic",
    "Clean AI traces in one click": "Limpiar rastros de IA en un clic",
    "Image delivery normalization": "Optimización de entrega de imágenes",
    "Visible-mark verification": "Verificación de marcas visibles",
  ...
  pt: {
    ...
    "One-click AI Trace Cleaner": "Limpeza de rastos de IA num clique",
    "Clean AI traces in one click": "Limpar rastos de IA num clique",
    "Image delivery normalization": "Otimização da entrega de imagens",
    "Visible-mark verification": "Verificação de marcas visíveis",
  ...
  id: {
    ...
    "One-click AI Trace Cleaner": "Pembersih Jejak AI Sekali Klik",
    "Clean AI traces in one click": "Bersihkan jejak AI sekali klik",
    "Image delivery normalization": "Optimasi pengiriman gambar",
    "Visible-mark verification": "Verifikasi tanda terlihat",
  ...
  de: {
    ...
    "One-click AI Trace Cleaner": "KI-Spuren mit einem Klick bereinigen",
    "Clean AI traces in one click": "KI-Spuren mit einem Klick bereinigen",
    "Image delivery normalization": "Optimierung der Bildausgabe",
    "Visible-mark verification": "Prüfung sichtbarer Markierungen",
  ...
  pl: {
    ...
    "One-click AI Trace Cleaner": "Usuwanie śladów AI jednym kliknięciem",
    "Clean AI traces in one click": "Usuń ślady AI jednym kliknięciem",
    "Image delivery normalization": "Optymalizacja obrazu do publikacji",
    "Visible-mark verification": "Weryfikacja widocznych oznaczeń",
  ...
  ru: {
    ...
    "One-click AI Trace Cleaner": "Удаление следов ИИ одним нажатием",
    "Clean AI traces in one click": "Удалить следы ИИ одним нажатием",
    "Image delivery normalization": "Оптимизация изображения для публикации",
    "Visible-mark verification": "Проверка видимых меток",
  ...
  fr: {
    ...
    "One-click AI Trace Cleaner": "Nettoyage des traces d’IA en un clic",
    "Clean AI traces in one click": "Nettoyer les traces d’IA en un clic",
    "Image delivery normalization": "Optimisation de la diffusion de l’image",
    "Visible-mark verification": "Vérification des marques visibles",
  ...
  ar: {
    ...
    "One-click AI Trace Cleaner": "تنظيف آثار الذكاء الاصطناعي بنقرة واحدة",
    "Clean AI traces in one click": "تنظيف آثار الذكاء الاصطناعي بنقرة واحدة",
    "Image delivery normalization": "تحسين الصورة للنشر",
    "Visible-mark verification": "التحقق من العلامات المرئية",
  ...
  tr: {
    ...
    "One-click AI Trace Cleaner": "Tek Tıkla AI İzi Temizleme",
    "Clean AI traces in one click": "AI izlerini tek tıkla temizle",
    "Image delivery normalization": "Görsel teslim optimizasyonu",
    "Visible-mark verification": "Görünür işaret doğrulaması",
...
// PDF workspace actions are short, context-sensitive commands. Keep these
// reviewed translations stable instead of relying on literal machine output.
const pdfWorkspaceOverrides = {
  ja: {
    "Page workspace": "ページ作業スペース", "PDF page workspace": "PDFページ作業スペース", "Add source PDFs": "PDFを追加", "Checking PDFs": "PDFを確認中", "Drop PDFs here, or click to choose": "PDFをここにドロップするか、クリックして選択", "Source files": "元のファイル", "Select pages": "ページを選択", "Select all": "すべて選択", "Clear selection": "選択を解除", "Reset order": "並び順をリセット", "Move selected pages earlier": "選択したページを前へ", "Move selected pages later": "選択したページを後ろへ", "Rotate left": "左に回転", "Rotate right": "右に回転", "Remove selected": "選択したページを削除", "Source page {page}": "元のページ {page}", "Hide source pages": "元のページを非表示", "Show source pages": "元のページを表示", "Remove source file": "元のファイルを削除", "Export complete PDF ({count} pages)": "PDF全体を書き出す（{count}ページ）", "Extract selected pages ({count})": "選択したページを書き出す（{count}ページ）", "Cancel export": "書き出しをキャンセル", "PDF created": "PDFを作成しました", "Unable to complete the action": "操作を完了できません", "Add one or more PDFs, then merge, reorder, rotate, remove, or extract pages in one workspace.": "1つ以上のPDFを追加し、同じ作業スペースで結合、並べ替え、回転、削除、ページ抽出を行えます。", "Saved": "削減率",
    "Add multiple PDFs to one page workspace, drag to reorder, batch-rotate, remove, or extract pages, and export in a background Worker. Image conversion remains available separately.": "複数のPDFを1つのページ作業スペースに追加し、ドラッグで並べ替え、一括回転、削除、ページ抽出を行い、バックグラウンドWorkerで書き出せます。画像変換は別のタブから利用できます。", "Source PDFs are never changed": "元のPDFは変更されません", "Thumbnails and exports run in background Workers": "サムネイルと書き出しはバックグラウンドWorkerで処理", "150 MB per file and 300 MB per workspace": "1ファイル150 MB、作業スペース合計300 MB",
  },
  ko: {
    "Page workspace": "페이지 작업 공간", "PDF page workspace": "PDF 페이지 작업 공간", "Add source PDFs": "PDF 추가", "Checking PDFs": "PDF 확인 중", "Drop PDFs here, or click to choose": "PDF를 여기에 놓거나 클릭하여 선택", "Source files": "원본 파일", "Select pages": "페이지 선택", "Select all": "모두 선택", "Clear selection": "선택 해제", "Reset order": "순서 초기화", "Move selected pages earlier": "선택한 페이지를 앞으로", "Move selected pages later": "선택한 페이지를 뒤로", "Rotate left": "왼쪽으로 회전", "Rotate right": "오른쪽으로 회전", "Remove selected": "선택한 페이지 삭제", "Source page {page}": "원본 페이지 {page}", "Hide source pages": "원본 페이지 숨기기", "Show source pages": "원본 페이지 표시", "Remove source file": "원본 파일 삭제", "Export complete PDF ({count} pages)": "전체 PDF 내보내기({count}페이지)", "Extract selected pages ({count})": "선택한 페이지 내보내기({count})", "Cancel export": "내보내기 취소", "PDF created": "PDF 생성 완료", "Unable to complete the action": "작업을 완료할 수 없습니다", "Add one or more PDFs, then merge, reorder, rotate, remove, or extract pages in one workspace.": "하나 이상의 PDF를 추가한 뒤 한 작업 공간에서 병합, 순서 변경, 회전, 삭제, 페이지 추출을 할 수 있습니다.",
    "Add multiple PDFs to one page workspace, drag to reorder, batch-rotate, remove, or extract pages, and export in a background Worker. Image conversion remains available separately.": "여러 PDF를 하나의 페이지 작업 공간에 추가해 드래그로 순서를 바꾸고, 일괄 회전·삭제·페이지 추출 후 백그라운드 Worker에서 내보낼 수 있습니다. 이미지 변환은 별도 탭에서 이용할 수 있습니다.", "Source PDFs are never changed": "원본 PDF는 변경되지 않습니다", "Thumbnails and exports run in background Workers": "미리보기와 내보내기는 백그라운드 Worker에서 처리", "150 MB per file and 300 MB per workspace": "파일당 150MB, 작업 공간 합계 300MB",
  },
  es: {
    "Page workspace": "Área de páginas", "PDF page workspace": "Área de páginas PDF", "Add source PDFs": "Añadir PDF", "Checking PDFs": "Comprobando PDF", "Drop PDFs here, or click to choose": "Suelta los PDF aquí o haz clic para elegir", "Source files": "Archivos de origen", "Select pages": "Seleccionar páginas", "Select all": "Seleccionar todo", "Clear selection": "Borrar selección", "Reset order": "Restablecer orden", "Move selected pages earlier": "Mover páginas seleccionadas hacia delante", "Move selected pages later": "Mover páginas seleccionadas hacia atrás", "Rotate left": "Girar a la izquierda", "Rotate right": "Girar a la derecha", "Remove selected": "Eliminar páginas seleccionadas", "Source page {page}": "Página de origen {page}", "Hide source pages": "Ocultar páginas de origen", "Show source pages": "Mostrar páginas de origen", "Remove source file": "Eliminar archivo de origen", "Export complete PDF ({count} pages)": "Exportar PDF completo ({count} páginas)", "Extract selected pages ({count})": "Extraer páginas seleccionadas ({count})", "Cancel export": "Cancelar exportación", "PDF created": "PDF creado", "Unable to complete the action": "No se pudo completar la acción", "Add one or more PDFs, then merge, reorder, rotate, remove, or extract pages in one workspace.": "Añade uno o varios PDF y combina, reordena, gira, elimina o extrae páginas en una sola área de trabajo.", "Process page ranges": "Procesar rangos de páginas",
    "Add multiple PDFs to one page workspace, drag to reorder, batch-rotate, remove, or extract pages, and export in a background Worker. Image conversion remains available separately.": "Añade varios PDF a una sola área de páginas, arrastra para reordenar, gira o elimina páginas en lote, extrae las que necesites y exporta en segundo plano. La conversión de imágenes sigue disponible por separado.", "Source PDFs are never changed": "Los PDF originales nunca se modifican", "Thumbnails and exports run in background Workers": "Las miniaturas y la exportación se procesan en segundo plano", "150 MB per file and 300 MB per workspace": "150 MB por archivo y 300 MB por área de trabajo",
  },
  pt: {
    "Page workspace": "Área de páginas", "PDF page workspace": "Área de páginas PDF", "Add source PDFs": "Adicionar PDF", "Checking PDFs": "A verificar PDF", "Drop PDFs here, or click to choose": "Largue os PDF aqui ou clique para escolher", "Source files": "Ficheiros de origem", "Select pages": "Selecionar páginas", "Select all": "Selecionar tudo", "Clear selection": "Limpar seleção", "Reset order": "Repor ordem", "Move selected pages earlier": "Mover páginas selecionadas para a frente", "Move selected pages later": "Mover páginas selecionadas para trás", "Rotate left": "Rodar para a esquerda", "Rotate right": "Rodar para a direita", "Remove selected": "Remover páginas selecionadas", "Source page {page}": "Página de origem {page}", "Hide source pages": "Ocultar páginas de origem", "Show source pages": "Mostrar páginas de origem", "Remove source file": "Remover ficheiro de origem", "Export complete PDF ({count} pages)": "Exportar PDF completo ({count} páginas)", "Extract selected pages ({count})": "Extrair páginas selecionadas ({count})", "Cancel export": "Cancelar exportação", "PDF created": "PDF criado", "Unable to complete the action": "Não foi possível concluir a ação", "Add one or more PDFs, then merge, reorder, rotate, remove, or extract pages in one workspace.": "Adicione um ou vários PDF e junte, reordene, rode, remova ou extraia páginas numa única área de trabalho.",
    "Add multiple PDFs to one page workspace, drag to reorder, batch-rotate, remove, or extract pages, and export in a background Worker. Image conversion remains available separately.": "Adicione vários PDF a uma única área de páginas, arraste para reordenar, rode ou remova páginas em lote, extraia as necessárias e exporte em segundo plano. A conversão de imagens continua disponível em separado.", "Source PDFs are never changed": "Os PDF originais nunca são alterados", "Thumbnails and exports run in background Workers": "As miniaturas e exportações são processadas em segundo plano", "150 MB per file and 300 MB per workspace": "150 MB por ficheiro e 300 MB por área de trabalho",
  },
  id: {
    "Page workspace": "Ruang kerja halaman", "PDF page workspace": "Ruang kerja halaman PDF", "Add source PDFs": "Tambah PDF", "Checking PDFs": "Memeriksa PDF", "Drop PDFs here, or click to choose": "Letakkan PDF di sini atau klik untuk memilih", "Source files": "File sumber", "Select pages": "Pilih halaman", "Select all": "Pilih semua", "Clear selection": "Hapus pilihan", "Reset order": "Atur ulang urutan", "Move selected pages earlier": "Pindahkan halaman terpilih ke depan", "Move selected pages later": "Pindahkan halaman terpilih ke belakang", "Rotate left": "Putar ke kiri", "Rotate right": "Putar ke kanan", "Remove selected": "Hapus halaman terpilih", "Source page {page}": "Halaman sumber {page}", "Hide source pages": "Sembunyikan halaman sumber", "Show source pages": "Tampilkan halaman sumber", "Remove source file": "Hapus file sumber", "Export complete PDF ({count} pages)": "Ekspor PDF lengkap ({count} halaman)", "Extract selected pages ({count})": "Ekspor halaman terpilih ({count})", "Cancel export": "Batalkan ekspor", "PDF created": "PDF berhasil dibuat", "Unable to complete the action": "Tindakan tidak dapat diselesaikan", "Add one or more PDFs, then merge, reorder, rotate, remove, or extract pages in one workspace.": "Tambahkan satu atau beberapa PDF, lalu gabungkan, ubah urutan, putar, hapus, atau ekspor halaman dalam satu ruang kerja.",
    "Add multiple PDFs to one page workspace, drag to reorder, batch-rotate, remove, or extract pages, and export in a background Worker. Image conversion remains available separately.": "Tambahkan beberapa PDF ke satu ruang kerja halaman, seret untuk mengubah urutan, putar atau hapus halaman secara massal, ekstrak halaman yang diperlukan, lalu ekspor di latar belakang. Konversi gambar tetap tersedia secara terpisah.", "Source PDFs are never changed": "PDF sumber tidak pernah diubah", "Thumbnails and exports run in background Workers": "Thumbnail dan ekspor diproses di latar belakang", "150 MB per file and 300 MB per workspace": "150 MB per file dan 300 MB per ruang kerja",
  },
  de: {
    "Page workspace": "Seiten-Arbeitsbereich", "PDF page workspace": "PDF-Seiten-Arbeitsbereich", "Add source PDFs": "PDF hinzufügen", "Checking PDFs": "PDFs werden geprüft", "Drop PDFs here, or click to choose": "PDFs hier ablegen oder zum Auswählen klicken", "Source files": "Quelldateien", "Select pages": "Seiten auswählen", "Select all": "Alle auswählen", "Clear selection": "Auswahl aufheben", "Reset order": "Reihenfolge zurücksetzen", "Move selected pages earlier": "Ausgewählte Seiten nach vorn", "Move selected pages later": "Ausgewählte Seiten nach hinten", "Rotate left": "Nach links drehen", "Rotate right": "Nach rechts drehen", "Remove selected": "Ausgewählte Seiten entfernen", "Source page {page}": "Quellseite {page}", "Hide source pages": "Quellseiten ausblenden", "Show source pages": "Quellseiten anzeigen", "Remove source file": "Quelldatei entfernen", "Export complete PDF ({count} pages)": "Vollständige PDF exportieren ({count} Seiten)", "Extract selected pages ({count})": "Ausgewählte Seiten exportieren ({count})", "Cancel export": "Export abbrechen", "PDF created": "PDF erstellt", "Unable to complete the action": "Aktion konnte nicht abgeschlossen werden", "Add one or more PDFs, then merge, reorder, rotate, remove, or extract pages in one workspace.": "Eine oder mehrere PDFs hinzufügen und Seiten in einem Arbeitsbereich zusammenführen, neu anordnen, drehen, entfernen oder exportieren.", "Process page ranges": "Seitenbereiche verarbeiten",
    "Add multiple PDFs to one page workspace, drag to reorder, batch-rotate, remove, or extract pages, and export in a background Worker. Image conversion remains available separately.": "Mehrere PDFs in einen Seiten-Arbeitsbereich laden, per Drag-and-drop sortieren, Seiten gesammelt drehen, entfernen oder extrahieren und im Hintergrund exportieren. Die Bildkonvertierung bleibt separat verfügbar.", "Source PDFs are never changed": "Die Original-PDFs werden nie verändert", "Thumbnails and exports run in background Workers": "Vorschaubilder und Export laufen im Hintergrund", "150 MB per file and 300 MB per workspace": "150 MB pro Datei und 300 MB pro Arbeitsbereich",
  },
  pl: {
    "Page workspace": "Obszar stron", "PDF page workspace": "Obszar stron PDF", "Add source PDFs": "Dodaj PDF", "Checking PDFs": "Sprawdzanie plików PDF", "Drop PDFs here, or click to choose": "Upuść pliki PDF tutaj lub kliknij, aby wybrać", "Source files": "Pliki źródłowe", "Select pages": "Wybierz strony", "Select all": "Zaznacz wszystko", "Clear selection": "Wyczyść zaznaczenie", "Reset order": "Przywróć kolejność", "Move selected pages earlier": "Przenieś zaznaczone strony do przodu", "Move selected pages later": "Przenieś zaznaczone strony do tyłu", "Rotate left": "Obróć w lewo", "Rotate right": "Obróć w prawo", "Remove selected": "Usuń zaznaczone strony", "Source page {page}": "Strona źródłowa {page}", "Hide source pages": "Ukryj strony źródłowe", "Show source pages": "Pokaż strony źródłowe", "Remove source file": "Usuń plik źródłowy", "Export complete PDF ({count} pages)": "Eksportuj cały PDF ({count} stron)", "Extract selected pages ({count})": "Eksportuj zaznaczone strony ({count})", "Cancel export": "Anuluj eksport", "PDF created": "Utworzono PDF", "Unable to complete the action": "Nie udało się wykonać działania", "Add one or more PDFs, then merge, reorder, rotate, remove, or extract pages in one workspace.": "Dodaj jeden lub kilka plików PDF, a następnie połącz, uporządkuj, obróć, usuń lub wyeksportuj strony w jednym obszarze roboczym.", "Decode component": "Dekoduj komponent", "How to use {name}": "Jak korzystać z narzędzia „{name}”",
    "Add multiple PDFs to one page workspace, drag to reorder, batch-rotate, remove, or extract pages, and export in a background Worker. Image conversion remains available separately.": "Dodaj wiele plików PDF do jednego obszaru stron, przeciągaj je, aby zmienić kolejność, grupowo obracaj lub usuwaj strony, wyodrębniaj wybrane i eksportuj w tle. Konwersja obrazów pozostaje dostępna osobno.", "Source PDFs are never changed": "Źródłowe pliki PDF nigdy nie są zmieniane", "Thumbnails and exports run in background Workers": "Miniatury i eksport są przetwarzane w tle", "150 MB per file and 300 MB per workspace": "150 MB na plik i 300 MB na obszar roboczy",
  },
  ru: {
    "Page workspace": "Рабочая область страниц", "PDF page workspace": "Рабочая область страниц PDF", "Add source PDFs": "Добавить PDF", "Checking PDFs": "Проверка PDF", "Drop PDFs here, or click to choose": "Перетащите PDF сюда или нажмите, чтобы выбрать", "Source files": "Исходные файлы", "Select pages": "Выбрать страницы", "Select all": "Выбрать все", "Clear selection": "Снять выделение", "Reset order": "Сбросить порядок", "Move selected pages earlier": "Переместить выбранные страницы вперёд", "Move selected pages later": "Переместить выбранные страницы назад", "Rotate left": "Повернуть влево", "Rotate right": "Повернуть вправо", "Remove selected": "Удалить выбранные страницы", "Source page {page}": "Исходная страница {page}", "Hide source pages": "Скрыть исходные страницы", "Show source pages": "Показать исходные страницы", "Remove source file": "Удалить исходный файл", "Export complete PDF ({count} pages)": "Экспортировать весь PDF ({count} стр.)", "Extract selected pages ({count})": "Экспортировать выбранные страницы ({count})", "Cancel export": "Отменить экспорт", "PDF created": "PDF создан", "Unable to complete the action": "Не удалось выполнить действие", "Add one or more PDFs, then merge, reorder, rotate, remove, or extract pages in one workspace.": "Добавьте один или несколько PDF и объединяйте, меняйте порядок, поворачивайте, удаляйте или экспортируйте страницы в одной рабочей области.", "Remove": "Удалить",
    "Add multiple PDFs to one page workspace, drag to reorder, batch-rotate, remove, or extract pages, and export in a background Worker. Image conversion remains available separately.": "Добавьте несколько PDF в одну рабочую область, перетаскивайте их для изменения порядка, массово поворачивайте или удаляйте страницы, извлекайте выбранные и экспортируйте в фоне. Преобразование изображений доступно отдельно.", "Source PDFs are never changed": "Исходные PDF никогда не изменяются", "Thumbnails and exports run in background Workers": "Миниатюры и экспорт обрабатываются в фоне", "150 MB per file and 300 MB per workspace": "150 МБ на файл и 300 МБ на рабочую область",
  },
  fr: {
    "Page workspace": "Espace de pages", "PDF page workspace": "Espace de pages PDF", "Add source PDFs": "Ajouter des PDF", "Checking PDFs": "Vérification des PDF", "Drop PDFs here, or click to choose": "Déposez les PDF ici ou cliquez pour les choisir", "Source files": "Fichiers sources", "Select pages": "Sélectionner des pages", "Select all": "Tout sélectionner", "Clear selection": "Effacer la sélection", "Reset order": "Réinitialiser l’ordre", "Move selected pages earlier": "Déplacer les pages sélectionnées vers l’avant", "Move selected pages later": "Déplacer les pages sélectionnées vers l’arrière", "Rotate left": "Pivoter à gauche", "Rotate right": "Pivoter à droite", "Remove selected": "Supprimer les pages sélectionnées", "Source page {page}": "Page source {page}", "Hide source pages": "Masquer les pages sources", "Show source pages": "Afficher les pages sources", "Remove source file": "Supprimer le fichier source", "Export complete PDF ({count} pages)": "Exporter le PDF complet ({count} pages)", "Extract selected pages ({count})": "Exporter les pages sélectionnées ({count})", "Cancel export": "Annuler l’exportation", "PDF created": "PDF créé", "Unable to complete the action": "Impossible d’effectuer l’action", "Add one or more PDFs, then merge, reorder, rotate, remove, or extract pages in one workspace.": "Ajoutez un ou plusieurs PDF, puis fusionnez, réorganisez, faites pivoter, supprimez ou exportez des pages dans un seul espace de travail.", "Remove": "Supprimer",
    "Add multiple PDFs to one page workspace, drag to reorder, batch-rotate, remove, or extract pages, and export in a background Worker. Image conversion remains available separately.": "Ajoutez plusieurs PDF dans un seul espace de pages, réorganisez-les par glisser-déposer, faites pivoter ou supprimez des pages par lot, extrayez les pages voulues et exportez en arrière-plan. La conversion d’images reste disponible séparément.", "Source PDFs are never changed": "Les PDF d’origine ne sont jamais modifiés", "Thumbnails and exports run in background Workers": "Les miniatures et l’exportation sont traitées en arrière-plan", "150 MB per file and 300 MB per workspace": "150 Mo par fichier et 300 Mo par espace de travail",
  },
  ar: {
    "Page workspace": "مساحة عمل الصفحات", "PDF page workspace": "مساحة عمل صفحات PDF", "Add source PDFs": "إضافة ملفات PDF", "Checking PDFs": "جارٍ فحص ملفات PDF", "Drop PDFs here, or click to choose": "أسقط ملفات PDF هنا أو انقر للاختيار", "Source files": "الملفات المصدرية", "Select pages": "تحديد الصفحات", "Select all": "تحديد الكل", "Clear selection": "إلغاء التحديد", "Reset order": "إعادة ضبط الترتيب", "Move selected pages earlier": "نقل الصفحات المحددة إلى الأمام", "Move selected pages later": "نقل الصفحات المحددة إلى الخلف", "Rotate left": "تدوير لليسار", "Rotate right": "تدوير لليمين", "Remove selected": "حذف الصفحات المحددة", "Source page {page}": "الصفحة المصدرية {page}", "Hide source pages": "إخفاء الصفحات المصدرية", "Show source pages": "إظهار الصفحات المصدرية", "Remove source file": "حذف الملف المصدري", "Export complete PDF ({count} pages)": "تصدير ملف PDF كاملاً ({count} صفحة)", "Extract selected pages ({count})": "تصدير الصفحات المحددة ({count})", "Cancel export": "إلغاء التصدير", "PDF created": "تم إنشاء ملف PDF", "Unable to complete the action": "تعذر إكمال الإجراء", "Add one or more PDFs, then merge, reorder, rotate, remove, or extract pages in one workspace.": "أضف ملف PDF واحداً أو أكثر، ثم ادمج الصفحات أو أعد ترتيبها أو دوّرها أو احذفها أو صدّرها في مساحة عمل واحدة.", "CPU threads": "خيوط المعالج", "Remove": "إزالة", "Merge PDFs": "دمج ملفات PDF", "Process page ranges": "معالجة نطاقات الصفحات",
    "Add multiple PDFs to one page workspace, drag to reorder, batch-rotate, remove, or extract pages, and export in a background Worker. Image conversion remains available separately.": "أضف عدة ملفات PDF إلى مساحة عمل واحدة، واسحب لتغيير الترتيب، ودوّر الصفحات أو احذفها دفعة واحدة، واستخرج الصفحات المطلوبة، ثم صدّر في الخلفية. يظل تحويل الصور متاحاً بشكل منفصل.", "Source PDFs are never changed": "لا يتم تغيير ملفات PDF المصدرية", "Thumbnails and exports run in background Workers": "تُعالج الصور المصغرة وعمليات التصدير في الخلفية", "150 MB per file and 300 MB per workspace": "150 ميجابايت لكل ملف و300 ميجابايت لكل مساحة عمل",
  },
  tr: {
...
const backgroundFinishOverrides = {
  ja: {
    "Finish image": "画像仕上げ",
    "Finish the image": "画像を仕上げる",
    "Blur original": "元画像をぼかす",
    "Canvas": "キャンバス",
    "Apply settings to the batch": "設定を一括適用",
    "Apply to current image": "現在の画像に適用",
    "Apply to all completed": "完了した画像すべてに適用",
    "Upload custom background": "カスタム背景をアップロード",
    "Preset backgrounds": "プリセット背景",
    "Select one to preview it. Every scene is generated instantly on this device.": "選ぶとプレビューできます。すべてのシーンはこのデバイス上ですぐに生成されます。",
    "White studio": "白いスタジオ",
    "Warm studio": "暖色スタジオ",
    "Cool studio": "クールグレーのスタジオ",
    "Pastel glow": "パステルライト",
    "Soft office": "ぼかしオフィス",
    "Warm café": "暖色カフェ",
    "Green nature": "緑の自然",
    "City dusk": "夕暮れの街",
    "Add subject shadow": "被写体に影を追加",
    "Composited entirely on this device": "このデバイス上でのみ合成",
    "Refine the cutout and finish the image": "切り抜きを調整して画像を仕上げる",
  },
  ko: {
    "Finish image": "이미지 마무리",
    "Finish the image": "이미지 마무리하기",
    "Blur original": "원본 흐리게",
    "Canvas": "캔버스",
    "Apply settings to the batch": "설정을 일괄 적용",
    "Apply to current image": "현재 이미지에 적용",
    "Apply to all completed": "완료된 모든 이미지에 적용",
    "Upload custom background": "사용자 배경 업로드",
    "Preset backgrounds": "기본 배경",
    "Select one to preview it. Every scene is generated instantly on this device.": "선택하면 바로 미리 볼 수 있습니다. 모든 장면은 이 기기에서 즉시 생성됩니다.",
    "White studio": "화이트 스튜디오",
    "Warm studio": "웜 스튜디오",
    "Cool studio": "쿨 그레이 스튜디오",
    "Pastel glow": "파스텔 빛",
    "Soft office": "소프트 오피스",
    "Warm café": "따뜻한 카페",
    "Green nature": "초록 자연",
    "City dusk": "도시의 황혼",
    "Add subject shadow": "피사체 그림자 추가",
    "Composited entirely on this device": "이 기기에서만 합성",
    "Refine the cutout and finish the image": "누끼 가장자리를 다듬고 이미지 마무리",
  },
  es: {
    "Finish image": "Finalizar imagen",
    "Finish the image": "Finalizar la imagen",
    "Blur original": "Desenfocar original",
    "Canvas": "Lienzo",
    "Apply settings to the batch": "Aplicar ajustes al lote",
    "Apply to current image": "Aplicar a la imagen actual",
    "Apply to all completed": "Aplicar a todas las completadas",
    "Upload custom background": "Subir fondo personalizado",
    "Preset backgrounds": "Fondos predeterminados",
    "Select one to preview it. Every scene is generated instantly on this device.": "Selecciona uno para previsualizarlo. Cada escena se genera al instante en este dispositivo.",
    "White studio": "Estudio blanco",
    "Warm studio": "Estudio cálido",
    "Cool studio": "Estudio gris frío",
    "Pastel glow": "Luz pastel",
    "Soft office": "Oficina desenfocada",
    "Warm café": "Cafetería cálida",
    "Green nature": "Naturaleza verde",
    "City dusk": "Ciudad al atardecer",
    "Add subject shadow": "Añadir sombra al sujeto",
    "Composited entirely on this device": "Compuesto íntegramente en este dispositivo",
    "Refine the cutout and finish the image": "Retoca el recorte y finaliza la imagen",
  },
  pt: {
    "Finish image": "Finalizar imagem",
    "Finish the image": "Finalizar a imagem",
    "Blur original": "Desfocar original",
    "Canvas": "Tela",
    "Apply settings to the batch": "Aplicar definições ao lote",
    "Apply to current image": "Aplicar à imagem atual",
    "Apply to all completed": "Aplicar a todas as concluídas",
    "Upload custom background": "Carregar fundo personalizado",
    "Preset backgrounds": "Fundos predefinidos",
    "Select one to preview it. Every scene is generated instantly on this device.": "Selecione um para pré-visualizar. Cada cenário é gerado de imediato neste dispositivo.",
    "White studio": "Estúdio branco",
    "Warm studio": "Estúdio acolhedor",
    "Cool studio": "Estúdio cinzento frio",
    "Pastel glow": "Luz pastel",
    "Soft office": "Escritório desfocado",
    "Warm café": "Café acolhedor",
    "Green nature": "Natureza verde",
    "City dusk": "Cidade ao entardecer",
    "Add subject shadow": "Adicionar sombra ao elemento principal",
    "Composited entirely on this device": "Composto inteiramente neste dispositivo",
    "Refine the cutout and finish the image": "Aperfeiçoe o recorte e finalize a imagem",
  },
  id: {
    "Finish image": "Selesaikan gambar",
    "Finish the image": "Selesaikan gambar",
    "Blur original": "Buramkan gambar asli",
    "Canvas": "Kanvas",
    "Apply settings to the batch": "Terapkan pengaturan ke semua gambar",
    "Apply to current image": "Terapkan ke gambar saat ini",
    "Apply to all completed": "Terapkan ke semua yang selesai",
    "Upload custom background": "Unggah latar khusus",
    "Preset backgrounds": "Latar siap pakai",
    "Select one to preview it. Every scene is generated instantly on this device.": "Pilih untuk melihat pratinjau. Setiap latar dibuat seketika di perangkat ini.",
    "White studio": "Studio putih",
    "Warm studio": "Studio hangat",
    "Cool studio": "Studio abu sejuk",
    "Pastel glow": "Cahaya pastel",
    "Soft office": "Kantor lembut",
    "Warm café": "Kafe hangat",
    "Green nature": "Alam hijau",
    "City dusk": "Kota senja",
    "Add subject shadow": "Tambahkan bayangan subjek",
    "Composited entirely on this device": "Disusun sepenuhnya di perangkat ini",
    "Refine the cutout and finish the image": "Rapikan potongan dan selesaikan gambar",
  },
  de: {
    "Finish image": "Bild fertigstellen",
    "Finish the image": "Bild fertigstellen",
    "Blur original": "Original weichzeichnen",
    "Canvas": "Arbeitsfläche",
    "Apply settings to the batch": "Einstellungen auf Stapel anwenden",
    "Apply to current image": "Auf aktuelles Bild anwenden",
    "Apply to all completed": "Auf alle abgeschlossenen Bilder anwenden",
    "Upload custom background": "Eigenen Hintergrund hochladen",
    "Preset backgrounds": "Vorgefertigte Hintergründe",
    "Select one to preview it. Every scene is generated instantly on this device.": "Zum Vorschauen auswählen. Jede Szene wird sofort auf diesem Gerät erzeugt.",
    "White studio": "Weißes Studio",
    "Warm studio": "Warmes Studio",
    "Cool studio": "Kühlgraues Studio",
    "Pastel glow": "Pastelllicht",
    "Soft office": "Weiches Büro",
    "Warm café": "Warmes Café",
    "Green nature": "Grüne Natur",
    "City dusk": "Stadt in der Dämmerung",
    "Add subject shadow": "Schatten zum Motiv hinzufügen",
    "Composited entirely on this device": "Vollständig auf diesem Gerät zusammengesetzt",
    "Refine the cutout and finish the image": "Freistellung verfeinern und Bild fertigstellen",
  },
  pl: {
    "Finish image": "Wykończ obraz",
    "Finish the image": "Wykończ obraz",
    "Blur original": "Rozmyj oryginał",
    "Canvas": "Obszar roboczy",
    "Apply settings to the batch": "Zastosuj ustawienia do całej partii",
    "Apply to current image": "Zastosuj do bieżącego obrazu",
    "Apply to all completed": "Zastosuj do wszystkich ukończonych",
    "Upload custom background": "Wczytaj własne tło",
    "Preset backgrounds": "Gotowe tła",
    "Select one to preview it. Every scene is generated instantly on this device.": "Wybierz tło, aby zobaczyć podgląd. Każda scena powstaje od razu na tym urządzeniu.",
    "White studio": "Białe studio",
    "Warm studio": "Ciepłe studio",
    "Cool studio": "Chłodne szare studio",
    "Pastel glow": "Pastelowe światło",
    "Soft office": "Rozmyte biuro",
    "Warm café": "Ciepła kawiarnia",
    "Green nature": "Zielona natura",
    "City dusk": "Miasto o zmierzchu",
    "Add subject shadow": "Dodaj cień obiektu",
    "Composited entirely on this device": "Złożono w całości na tym urządzeniu",
    "Refine the cutout and finish the image": "Popraw wycięcie i wykończ obraz",
  },
  ru: {
    "Finish image": "Финальная обработка",
    "Finish the image": "Завершить обработку изображения",
    "Blur original": "Размыть исходное изображение",
    "Canvas": "Холст",
    "Apply settings to the batch": "Применить настройки ко всей серии",
    "Apply to current image": "Применить к текущему изображению",
    "Apply to all completed": "Применить ко всем готовым изображениям",
    "Upload custom background": "Загрузить свой фон",
    "Preset backgrounds": "Готовые фоны",
    "Select one to preview it. Every scene is generated instantly on this device.": "Выберите фон для предпросмотра. Каждая сцена мгновенно создаётся на этом устройстве.",
    "White studio": "Белая студия",
    "Warm studio": "Тёплая студия",
    "Cool studio": "Холодная серая студия",
    "Pastel glow": "Пастельное сияние",
    "Soft office": "Размытый офис",
    "Warm café": "Уютное кафе",
    "Green nature": "Зелёная природа",
    "City dusk": "Город в сумерках",
    "Add subject shadow": "Добавить тень объекта",
    "Composited entirely on this device": "Полностью собрано на этом устройстве",
    "Refine the cutout and finish the image": "Уточните вырезанный объект и завершите обработку",
  },
  fr: {
    "Finish image": "Finaliser l’image",
    "Finish the image": "Finaliser l’image",
    "Blur original": "Flouter l’original",
    "Canvas": "Zone de travail",
    "Apply settings to the batch": "Appliquer les réglages au lot",
    "Apply to current image": "Appliquer à l’image actuelle",
    "Apply to all completed": "Appliquer à toutes les images terminées",
    "Upload custom background": "Importer un arrière-plan personnalisé",
    "Preset backgrounds": "Arrière-plans prédéfinis",
    "Select one to preview it. Every scene is generated instantly on this device.": "Sélectionnez-en un pour l’aperçu. Chaque scène est générée instantanément sur cet appareil.",
    "White studio": "Studio blanc",
    "Warm studio": "Studio chaleureux",
    "Cool studio": "Studio gris froid",
    "Pastel glow": "Lueur pastel",
    "Soft office": "Bureau flouté",
    "Warm café": "Café chaleureux",
    "Green nature": "Nature verdoyante",
    "City dusk": "Ville au crépuscule",
    "Add subject shadow": "Ajouter une ombre au sujet",
    "Composited entirely on this device": "Composé entièrement sur cet appareil",
    "Refine the cutout and finish the image": "Affinez le détourage et finalisez l’image",
  },
  ar: {
    "Finish image": "إنهاء الصورة",
    "Finish the image": "إنهاء الصورة",
    "Blur original": "تمويه الصورة الأصلية",
    "Canvas": "لوحة العمل",
    "Apply settings to the batch": "تطبيق الإعدادات على الدفعة",
    "Apply to current image": "تطبيق على الصورة الحالية",
    "Apply to all completed": "تطبيق على جميع الصور المكتملة",
    "Upload custom background": "رفع خلفية مخصصة",
    "Preset backgrounds": "خلفيات جاهزة",
    "Select one to preview it. Every scene is generated instantly on this device.": "اختر خلفية لمعاينتها. يتم إنشاء كل مشهد فوراً على هذا الجهاز.",
    "White studio": "استوديو أبيض",
    "Warm studio": "استوديو دافئ",
    "Cool studio": "استوديو رمادي بارد",
    "Pastel glow": "إضاءة باستيل",
    "Soft office": "مكتب ضبابي",
    "Warm café": "مقهى دافئ",
    "Green nature": "طبيعة خضراء",
    "City dusk": "مدينة عند الغسق",
    "Add subject shadow": "إضافة ظل للعنصر",
    "Composited entirely on this device": "تم التركيب بالكامل على هذا الجهاز",
    "Refine the cutout and finish the image": "حسّن حدود القص وأنهِ الصورة",
  },
  tr: {
    "Finish image": "Görseli tamamla",
    "Finish the image": "Görseli tamamla",
    "Blur original": "Orijinali bulanıklaştır",
    "Canvas": "Çalışma alanı",
    "Apply settings to the batch": "Ayarları toplu uygula",
    "Apply to current image": "Geçerli görsele uygula",
    "Apply to all completed": "Tamamlananların tümüne uygula",
    "Upload custom background": "Özel arka plan yükle",
    "Preset backgrounds": "Hazır arka planlar",
    "Select one to preview it. Every scene is generated instantly on this device.": "Önizlemek için birini seçin. Her sahne bu cihazda anında oluşturulur.",
    "White studio": "Beyaz stüdyo",
    "Warm studio": "Sıcak stüdyo",
    "Cool studio": "Soğuk gri stüdyo",
    "Pastel glow": "Pastel ışık",
    "Soft office": "Yumuşak ofis",
    "Warm café": "Sıcak kafe",
    "Green nature": "Yeşil doğa",
    "City dusk": "Alacakaranlık şehir",
    "Add subject shadow": "Özneye gölge ekle",
    "Composited entirely on this device": "Tamamı bu cihazda birleştirildi",
    "Refine the cutout and finish the image": "Kesimi düzelt ve görseli tamamla",
  },
}
