...
describe("tool navigation priority", () => {
  ...
  it("keeps the rotating homepage banner complete and navigable", () => {
    ...
    expect(homeHeroSlides[0].title.zh).toContain("一次搞定")
    expect(homeHeroSlides[0].description.zh).toContain("去背景")
    expect(homeHeroSlides[0].description.zh).toContain("修图")
    expect(homeHeroSlides[0].description.zh).toContain("切换功能不会清空图片")
    expect(homeHeroSlides[0].description.zh).toContain("不上传文件")
