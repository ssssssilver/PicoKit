...
describe("local alpha-mask refinement", () => {
  ...
  it("shows the refinement brush and source previews while keeping provenance outside the edit flow", async () => {
    ...
    expect(header).toContain('href="/remove-background"')
    expect(header).not.toContain('href="/ai-image-detector"')
  ...
  it("processes background-removal batches sequentially and packages completed results", async () => {
    ...
    expect(batch).toContain("deliveryName(item.file.name, item.result)")
    ...
    expect(batch).toContain("result: { ...current.result, blob, url, finish }")
    expect(batch).toContain("queue preview, downloads, ZIP")
    expect(batch).toContain("Refine the cutout and finish the image")
    expect(batch).toContain("Edit the first result")
    expect(batch).toContain('pick("继续编辑", "Continue editing")')
    expect(batch).toContain("<BackgroundFinishEditor")
    expect(batch).toContain("applyFinishedResultToBatch")
    expect(batch).toContain("deliveryResult(item.result)")
    expect(batch).toContain("deliveryName(item.file.name, item.result)")
    expect(batch).toContain('defaultValue="finish"')
    ...
    expect(batch).toContain("const pickRef = useRef(pick)")
    expect(batch).toContain("}, [workflowMemory])")
    expect(batch).not.toContain("}, [pick, workflowMemory])")
    expect(batch).toContain("await waitForBrowserPaint()")
    expect(batch).toContain("flushSync(() =>")
    expect(batch).toContain("aria-busy={adding || loadingSample}")
    expect(batch).toContain('aria-live="polite"')
    expect(batch).toContain('sample.src = "/samples/remove-background-cat.png"')
    expect(batch).toContain('canvas.toBlob(')
    expect(batch).not.toContain("fetch(")
    expect(batch).toContain('new File([blob], "tabnative-remove-background-sample.png"')
    expect(batch).toContain('pick("试用示例", "Try sample")')
    expect(batch).toContain('pick("正在载入示例", "Loading sample")')
    expect(batch).toContain("workflow-preview-action")
    expect(batch).toContain("Select any result to restore or erase mask areas")
