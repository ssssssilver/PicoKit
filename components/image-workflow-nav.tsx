...
export function ImageWorkflowNav({ active }: { active: WorkflowStep }) {
  ...
  return (
    <section className="mb-6 overflow-hidden rounded-xl border border-cyan-300/20 bg-cyan-300/[.035]" aria-label={pick("本地图片交付流程", "Local image delivery workflow")}>
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 px-4 py-3">
        <div>
          <p className="text-sm font-semibold text-zinc-100">{pick("本地图片交付流程", "Local image delivery workflow")}</p>
