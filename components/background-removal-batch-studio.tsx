...
export function BackgroundRemovalBatchStudio({
  ...
  return <div className="space-y-6">
    <Card className="border-cyan-300/20 bg-cyan-300/[.035] shadow-none">
      <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Images className="size-4 text-cyan-300" />{embedded ? pick("去背景与成品", "Background removal and finishing") : pick("批量移除背景", "Batch background removal")}</CardTitle><p className="text-sm text-zinc-500">{pick("一次加入多张图片，系统会逐张处理，数量多也不容易卡住页面。", "Add multiple images and process them sequentially to limit GPU and memory use.")}</p></CardHeader>
